...
public class ZKConfigManager {
    ...
    private static final String COUNTRY_ZK_PATH = "/config/country";
    ...
    //WEB IM 公网IP地址
    @Getter
    private Router webIMRouter;

    //直连的区域
    @Getter
    private String directLinkAreas;

    // 加速区域
    @Getter
    private String speedUpAreas;

    // 加速是否生效
    @Getter
    private boolean speedIsValid;
    ...
    @Getter
    private boolean gatewayHostPortEnable = true;
    ...
    private ZKConfigManager() {
         ...
         */
        IZkDataListener globalListener = new IZkDataListener() {
        ...
        IZkDataListener globalTCPListener = new IZkDataListener() {
            @Override
            public void handleDataChange(String dataPath, byte[] data) throws Exception {
                log.info("!!! configZkPath node data has been changed !!!" + dataPath);
                String rtdata = null;
        ...
        //添加节点监控
        zk.subscribeDataChange(CONFIG_ZK_PATH, globalListener);
        zk.subscribeDataChange(CONFIG_ZK_PATH_TCP, globalTCPListener);
        ...
        // 添加 country 节点监控
        zk.subscribeDataChange(COUNTRY_ZK_PATH, listenerCountry);

        IZkDataListener routerListener = new IZkDataListener() {
            ...
            public void handleDataChange(String dataPath, byte[] data) throws Exception {
                ...
                if(data != null && data.length > 0) {
                    rtData = new String(data, StandardCharsets.UTF_8);
                    JSONObject job = JSON.parseObject(rtData);
                    RouterConfig routerConfig = job.getObject("routerConfig", RouterConfig.class);
                    if(routerConfig != null){
                        ZKConfigManager.this.routerConfig = routerConfig;
                    }

                    Router webIMRouter = job.getObject("webIMRouter", Router.class);
                    if(webIMRouter != null){
                        ZKConfigManager.this.webIMRouter =  webIMRouter;
                    }

                    String directLinkAreas = job.getString("directLinkAreas");
                    if(StrUtil.isNotEmpty(directLinkAreas)){
                        ZKConfigManager.this.directLinkAreas = directLinkAreas;
                    }

                    String speedUpAreas = job.getString("speedUpAreas");
                    if(StrUtil.isNotEmpty(speedUpAreas)){
                        ZKConfigManager.this.speedUpAreas = speedUpAreas;
                    }

                    String diyJson = job.getString("diyJson");
                    if(StrUtil.isNotEmpty(diyJson)){
                        ZKConfigManager.this.diyJson = diyJson;
                    }

                    ZKConfigManager.this.gatewayHostPortEnable = job.getBooleanValue("gatewayHostPortEnable");
                }
                log.info("configZkPath node data has been changed ok. {} kafkaConfig: [{}]", rtData, routerConfig);
            }

            @Override
            public void handleDataDeleted(String dataPath) throws Exception {
                log.info("configZkPath node dataPath has been delete. {} kafkaConfig: [{}]", dataPath, routerConfig);
            }
        };
        // 添加 Router 节点监控
        zk.subscribeDataChange(ROUTER_ZK_PATH, routerListener);
                ...
                for(CountryRouterConfig crc : countryRouterConfigs){
                    ...
                    if(zk.exists(path)){
                        IZkDataListener listener = new IZkDataListener() {
                            ...
                            @Override
                            public void handleDataDeleted(String dataPath) throws Exception {
                                log.info("{}: {}", ROUTER_ZK_PATH, dataPath);
                            }
                        };
